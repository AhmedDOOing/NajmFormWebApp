...
async function post(body: object) {
  ...
  return j as { partyA?: { url?: string }; partyB?: { url?: string }; reportId?: string };
}
...
async function main() {
  ...
  const ended = await post(
    envelope("call.ended", {
      ...
      outcomeResult: {
        ...
        other_party_name: "سالم فهد العتيبي",
  ...
  console.log(
    `\n✔ Done. Party A link: ${ended.partyA?.url ?? "(none)"}\n  Party B link: ${ended.partyB?.url ?? "(none)"}\n  Watch: ${BASE}/phone`
  );
}
