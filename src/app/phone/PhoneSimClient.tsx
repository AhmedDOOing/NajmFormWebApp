...
const EVENT_STYLE: Record<string, { icon: typeof Radio; tone: string; label: string }> = {
  ...
  sms_causer: { icon: Send, tone: "text-primary border-primary/50", label: "sms" },
  sms_affected: { icon: Send, tone: "text-primary border-primary/50", label: "sms" },
  report_filed: { icon: Activity, tone: "text-primary border-primary/50", label: "action" },
  ack_accepted: { icon: Activity, tone: "text-primary border-primary/50", label: "action" },
  ack_rejected: { icon: Activity, tone: "text-destructive border-destructive/50", label: "action" },
};
...
function PhoneFrame({ number, messages }: { number: string; messages: Sms[] }) {
  const party = messages[messages.length - 1]?.toParty ?? "causer";
  const label =
    party === "causer" ? "هاتف المتسبب · Causer" : "هاتف المتضرر · Affected";
