...
- **causerFiling** — filing link, sent on `call.ended`.
- **affectedAck** — acknowledgment link, sent per affected party on submit.

Where they're triggered: `src/app/api/webhook/hamsa/route.ts` (causer) and
`src/app/api/report/[reportId]/submit/route.ts` (affected).
